  ...
  function injectDecorations() {
    ...
    sections.forEach((section, index) => {
      ...
      positions.forEach((position) => {
        ...
        const patternIndex =
          (index + positions.indexOf(position)) % svgPatterns.length;
  ...
  function init() {
    const runDecorations = () => {
      if (typeof window.requestIdleCallback === "function") {
        window.requestIdleCallback(() => injectDecorations(), {
          timeout: 2000,
        });
      } else {
        window.setTimeout(() => injectDecorations(), 0);
      }
    };
    ...
    if (document.readyState === "complete") {
      ...
      window.addEventListener("load", runDecorations, { once: true });
    }
  }
